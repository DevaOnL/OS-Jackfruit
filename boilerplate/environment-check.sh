...
if [[ -r /etc/os-release ]]; then
    ...
    if [[ "${ID:-}" == "ubuntu" ]]; then
        case "${VERSION_ID:-}" in
            22.04|24.04)
                OFFICIAL_SPEC_ENV=1
                ok "Ubuntu version ${VERSION_ID} detected."
                ;;
            *)
        ...
        LOCAL_CACHYOS_ENV=1
        warn "Detected ${PRETTY_NAME:-${ID:-unknown}}. The original assignment guide names Ubuntu 22.04/24.04 VM, but continuing with local CachyOS-compatible checks."
    else
        warn "Detected ${PRETTY_NAME:-${ID:-unknown}}. This is outside the original assignment guide; continuing with generic local checks."
    fi
else
...
if command -v systemd-detect-virt >/dev/null 2>&1; then
    ...
    elif [[ "$OFFICIAL_SPEC_ENV" -eq 1 ]]; then
        fail "No VM detected. Use an Ubuntu VM for this project."
    else
        warn "No VM detected. Continuing because this is a local non-Ubuntu development environment."
    fi
else
...
if [[ ! -d "$KBUILD_DIR" ]]; then
    if [[ "$LOCAL_CACHYOS_ENV" -eq 1 ]]; then
        fail "Kernel headers missing for $(uname -r). Install the matching CachyOS headers package, such as linux-cachyos-headers."
    fi
    fail "Kernel headers missing for $(uname -r). Install linux-headers-$(uname -r)."
fi
